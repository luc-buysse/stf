...
# Added on top of the regular STF implementation
# Adapters allow to adapt the bit rate without changing the model
#   only updating a reduced number of parameters
...
class SymmetricalTransFormer(CompressionModel):
    ...
    def forward(self, x):
        ...
        # print(torch.max(z).item(), torch.max(z_offset).item(), math.log(torch.min(z_likelihoods).item()))
